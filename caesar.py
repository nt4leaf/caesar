...
step_encode = st.number_input('Input step:', step = 1)
...
step_decode = st.number_input('Input step:', step = 1)
...
for i in range(len(data_decode)):
  ...
  if data_decode[i].islower():
    data_decode[i] = chr((ord(data_decode[i]) + step_decode - 97) % 26 + 97)
  if data_decode[i].isupper():
    data_decode[i] = chr((ord(data_decode[i]) + step_decode - 65) % 26 + 65)
